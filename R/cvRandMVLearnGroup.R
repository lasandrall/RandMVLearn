...
#' @title Cross-validation for randomized multiview learning with group information
#'
#' @description Performs nfolds cross-validation to select optimal tuning parameter for randomized
...
#' @param rhoLower A list of \eqn{d}  lower bound values for  \eqn{\rho >0}.  \eqn{\rho >0} controls the amount of sparsity,
#' for a fixed \eqn{\eta}. Default is \eqn{10^-7}.
#' @param rhoUpper list of \eqn{d} upper bound values. \eqn{\rho >0} controls the amount of sparsity, for a fixed \eqn{\eta}.
...
#' @param numbercores Number of cores to be used for parallel computing.  Defaults to half the size of the system cores.
#' @param gridMethod GridSearch or RandomSearch. Optimize tuning parameters over full grid or random grid. Default is Random Search.
#' @param nfolds A list of  \eqn{d}  integers specifying the kernel parameters for the Gaussian kernel.
#' If NULL, algorithm will choose kernel parameters for each view using median heuristic.
#' @param ngrid A list of  \eqn{d}  integers specifying the regularization parameters controlling the
#' trade-off between model fit and complexity. Default is 1 for each view.
...
#'
#' @seealso \code{\link{generateData},\link{RandMVPredict},\link{RandMVLearnGroup},\link{RandMVLearnR}}
#'
#' @author
#' Sandra E. Safo
#'
...
if(is.null(hasGroupInfo)){
  ...
  #   numbercores=as.integer(ceiling(ncores/2)+1)
  # }
  if (Sys.getenv("_R_CHECK_LIMIT_CORES_", "TRUE") == "TRUE") {
    numbercores <- 2  # Force limit to 2 cores
  } else {
    ncores <- detectCores()
    numbercores <- max(1, as.integer(ceiling(ncores / 2) + 1))  # At least 1 core
  }

  # Create cluster using the determined core count (never exceed 2 in R CMD check)
  cl <- parallel::makeCluster(numbercores)
  doParallel::registerDoParallel(cl)
  on.exit(parallel::stopCluster(cl), add = TRUE)

  myrmvlearn2=nsrf_cvindgrouplassoNew(Xdata, Y,hasGroupInfo,GroupIndices,myseed,
                                      myeta, rhoLower, rhoUpper,ncomponents,
                                      num_features,outcometype,kernel_param,mylambda,numbercores,
                                     gridMethod,nfolds,ngrid,max_iter_nsir, max_iter_PG, update_thresh_nsir,
                                     update_thresh_PG,standardize_Y,standardize_X,omegaweight)
